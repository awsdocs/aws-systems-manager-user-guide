# Troubleshooting managed instance availability<a name="troubleshooting-managed-instances"></a>

For several AWS Systems Manager capabilities like Run Command, Distributor, and Session Manager, you can choose to manually select the instances on which you want to run an operation\. In cases like these, after you specify that you want to choose instances manually, the system displays a list of managed instances where you can run the operation\.

This topic provides information to help you diagnose why an Amazon Elastic Compute Cloud \(Amazon EC2\) or on\-premises instance *that you have confirmed is running* isn't included in your lists of managed instances in Systems Manager\. 

In order for an instance to be managed by Systems Manager and available in lists of managed instances, it must meet three primary requirements:
...
+ An AWS Identity and Access Management \(IAM\) instance profile that supplies the required permissions for the instance to communicate with the Systems Manager service must be attached to the instance, or it must be registered using a hybrid activation\.
...
After you verify that an instance is running, you can use the following command to check whether SSM Agent on one of these instance types has successfully registered itself with the Systems Manager service\. This command doesn't return results until a successful registration has taken place\.
...
To determine whether SSM Agent is installed and running on an instance, see [Checking SSM Agent status and starting the agent](ssm-agent-status-and-restart.md)\.

To install or reinstall SSM Agent on an instance, see the following topics:
...
Verify that the Amazon EC2 instance is configured with an AWS Identity and Access Management \(IAM\) role that allows the instance to communicate with the Systems Manager API\. Also verify that your user account has an IAM user trust policy that allows your account to communicate with the Systems Manager API\. Though you can't attach instance profiles to on\-premises machines the IAM service role you created for the hybrid activation must provide the required permissions\. For more information about creating IAM service roles for on\-premises machines, see [Create an IAM service role for a hybrid environment](sysman-service-role.md)\.
