...
You can choose to store session log data in a specified S3 bucket for debugging and troubleshooting purposes\. The default option is for logs to be sent to an encrypted S3 bucket\. Encryption is performed using the key specified for the bucket, either an AWS KMS key or an Amazon S3 Server\-Side Encryption \(SSE\) key \(AES\-256\)\. 
