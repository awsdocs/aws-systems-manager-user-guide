...
**Note**  
When using GET calls, parameter values must be encoded for HTTP to preserve special characters\. For example, instead of `/abc/def/hij/`, use `%2Fabc%2Fdef%2Fhij%2F`\.
...
```
GET: /systemsmanager/parameters/get?name=%2Fabc%2Fdef%2Fhij&label=release
```

To call a public \(global\) parameter, make an HTTP GET call similar to the following\.

```
GET: /systemsmanager/parameters/get/?name=%2Faws%2Fservice%2Flist%2F…
```
...
```
GET: /systemsmanager/parameters/get?name=%2Faws%2Freference%2Fsecretsmanager%2F…
```

To make a call using the Amazon Resource Name \(ARN\) for a parameter, make an HTTP GET call similar to the following\.

```
GET: /systemsmanager/parameters/get?name=arn:aws:ssm:us-east-1:12345678910:parameter%2FMyParameter
```
