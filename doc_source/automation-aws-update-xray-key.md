...
+ KeyId
  ...
  Description: \(Required\) The Amazon Resource Name \(ARN\), key ID, or the key alias of the AWS Key Management Service \(AWS KMS\) key you want AWS X\-Ray to use to encrypt data\.
...
**Document Steps**
+ aws:executeAwsApi \- Enables encryption on your X\-Ray data using the AWS KMS key you specify in the `KeyId` parameter\.
