...
The legacy document `AWS-ApplyPatchBaseline` applies only to Windows Server instances, and doesn't provide support for application patching\. The newer `AWS-RunPatchBaseline` provides the same support for both Windows and Linux systems\. Version 2\.0\.834\.0 or later of SSM Agent is required in order to use the `AWS-RunPatchBaseline` document\. 
...
Supports only Windows Server instances, but doesn't include support for patching applications that is found in its replacement, `AWS-RunPatchBaseline`\. Not available in AWS Regions launched after August 2017\.
