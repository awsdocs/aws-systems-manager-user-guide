...
|  instanceIds  |  StringList  |  The instances that were targeted by the command\.  Instance IDs are only included in the summary message if the Run Command task targeted instance IDs directly\. Instance IDs aren't included in the summary message if the Run Command task was issued using tag\-based targeting\.   | 
...
| Field | Type | Description | 
| --- | --- | --- | 
|  eventTime  |  String  |  The time that the event was triggered\. The timestamp is important because Amazon SNS doesn't guarantee message delivery order\. Example: 2016\-04\-26T13:15:30Z   | 
