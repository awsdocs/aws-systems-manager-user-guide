...
+  **AWS PrivateLink support for managed nodes without public IP addresses** 
  ...
  You can also set up VPC Endpoints for Systems Manager using AWS PrivateLink to further secure your sessions\. AWS PrivateLink limits all network traffic between your managed nodes, Systems Manager, and Amazon EC2 to the Amazon network\. For more information, see [\(Optional\) Create a VPC endpoint](setup-create-vpc.md)\.
+  **Tunneling** 
