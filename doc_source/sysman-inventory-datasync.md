...
Use the following procedure to create a resource data sync for Systems Manager Inventory by using the Systems Manager console\. For information about how to create a resource data sync by using the AWS CLI, see [Walkthrough: Configure your managed nodes for Inventory by using the CLI](sysman-inventory-cliwalk.md)\.
