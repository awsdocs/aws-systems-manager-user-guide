...
In the system, change requests are a type of OpsItem in AWS Systems Manager OpsCenter \(OpsCenter\)\. However, OpsItems of type the type`/aws/changerequest` are not displayed in OpsCenter\. As OpsItems, change requests are subject to the same enforced limits as other types of OpsItems\. For information about the number of OpsItems that can be created for an AWS account in an AWS Region, see [What are the resource limits for OpsCenter?](OpsCenter.md#OpsCenter-learn-more-limits)\.
