...
+ **Integration with AWS Single Sign\-On and Active Directory support**
  ...
  With AWS SSO integration, members of your organization can access Amazon Web Services accounts and manage their resources using Systems Manager based on a common user identity\. Using AWS SSO, you can assign your users access to accounts across AWS\. 
...
In the system, change requests are a type of OpsItem in AWS Systems Manager OpsCenter\. However, OpsItems of type the type`/aws/changerequest` are not displayed in OpsCenter\. As OpsItems, change requests are subject to the same enforced limits as other types of OpsItems\. For information about the number of OpsItems that can be created for an Amazon Web Services account in an AWS Region, see [What are the quotas for OpsCenter?](OpsCenter.md#OpsCenter-learn-more-limits)\.
