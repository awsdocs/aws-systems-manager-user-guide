...
1. In the **Configuration options** section, choose the options that you want to allow for your configuration\.
   ...
   If you choose the **Update Systems Manager \(SSM\) Agent every two weeks** option, then Systems Manager automatically checks every two weeks for a new version of the agent\. If there is a new version, then Systems Manager automatically updates the agent on your managed node to the latest released version\. We encourage you to choose this option to ensure that your nodes are always running the most up\-to\-date version of SSM Agent\. For more information about SSM Agent, including information about how to manually install the agent, see [Working with SSM Agent](ssm-agent.md)\.
   ...
   If you choose the **Scan instances for missing patches daily** option, then Systems Manager uses Patch Manager to scan your nodes each day and generate a simple report in the **Compliance** page\. Patch Manager is a capability of AWS Systems Manager\. The report shows how many nodes are patch\-compliant according to the *default patch baseline*\. The report includes a list of each node and its compliance status\. You can navigate this list to see details about noncompliant nodes\. For more information about patching operations and patch baselines, see [AWS Systems Manager Patch Manager](systems-manager-patch.md)\. To view compliance information, see the Systems Manager [Compliance](https://console.aws.amazon.com/systems-manager/compliance) page\.
...
For each option you selected, Systems Manager creates and immediately runs a State Manager association\. The **Configuration status** field shows **Success** when the association successfully runs on *all* selected or targeted nodes\. If one association fails to run, **Configuration status** is **Failed**\. If an association is processing, **Configuration status** is **Pending**\. To update **Configuration status** for **Pending** items, refresh your browser\.
...
**Important**  
If you want to run Quick Setup on new nodes, we recommend that you choose the new nodes and also choose all of the nodes on which you previously ran Quick Setup\. By choosing all of the nodes on which you previously ran Quick Setup, you synchronize the association executions for all of the nodes\. This synchronizes status and compliance reporting\. We also recommend that you target nodes by using tags\. New nodes with the specified tags are *automatically* added to Quick Setup associations\. This means that they automatically display status in the Quick Setup results and in the **Compliance** page\.

If you want to delete your Quick Setup from a specific Region, delete the associations from the Quick Setup results page or from the **State Manager** page in that Region\. You can also remove Regions from a configuration by choosing **Remove Regions** on the configuration details page under the **Actions** dropdown\.