# Systems Manager resources you can tag<a name="taggable-resources"></a>
...
You can also restrict user access to these resource types by creating AWS Identity and Access Management \(IAM\) policies that specify the tags that a user can access and attaching the policy to IAM entities \(users, roles, or groups\)\. The following are several examples of restricting resource access using tags\. 
+ You can apply a tag to a set of custom Systems Manager documents \(SSM documents\) and then create and apply an IAM policy that grants access to documents with that tag but no others \(or that prohibits access to only those documents\)\.
